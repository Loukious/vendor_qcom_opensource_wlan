...
// lct.dongxiaolong modify for scc begin
static QDF_STATUS
...
// lct.dongxiaolong modify for scc end
		...
		uint32_t *pcl_len_org)
{
	...
	if (!skip_dfs_channel) {
		if (policy_mgr_get_connection_count(psoc) == 1) {
			freq = pm_conc_connection_list[0].freq;
			policy_mgr_debug("allow sap create on dfs freq=%u", freq);
		}
		for (i = 0; i < *pcl_len_org; i++) {
				      ...
				      uint32_t *pcl_len_org)
{
	...
	if (include_indoor_channel) {
		if (policy_mgr_get_connection_count(psoc) == 1) {
			freq = pm_conc_connection_list[0].freq;
			policy_mgr_debug("allow sap create on indoor freq %u", freq);
		}
		for (i = 0; i < *pcl_len_org; i++) {
		if (wlan_reg_is_freq_indoor_in_secondary_list(pm_ctx->pdev,
							pcl_list_org[i]) &&
	...
	// lct.dongxiaolong modify for pcl_list begin
	// lct.dongxiaolong modify for pcl_list end
