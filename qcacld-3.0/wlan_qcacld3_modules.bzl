...
def _define_module_for_target_variant_chipset(target, variant, chipset):
    ...
    if target == "suiren" or target == "ampere":
        copts.append("-DQC_WIFI_VENDOR_AUTO")

    for f in _xiaomi_feature_copts_map:
        for t in _xiaomi_feature_copts_map.get(f):
            if target == t:
                copts.append("-D{}".format(f))
                break
