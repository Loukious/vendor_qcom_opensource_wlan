				    ...
				    uint8_t *num_ch)
{
	...
	uint32_t link0_work_freq = 0,link1_work_freq = 0;
	...
	if (mac_ctx->psoc) {
		if (policy_mgr_get_connection_count(mac_ctx->psoc) == 1) {
			link0_work_freq = pm_conc_connection_list[0].freq;
			sap_debug("allow sap to use link0_work_freq %u", link0_work_freq);
		} else if (policy_mgr_get_connection_count_with_mlo(mac_ctx->psoc) == 1){
			 if ((pm_conc_connection_list[0].mode == PM_STA_MODE) && pm_conc_connection_list[0].in_use) {
				  link0_work_freq = pm_conc_connection_list[0].freq;
				  sap_debug("allow sap to use link0_work_freq %u", link0_work_freq);
			 }
			 if ((pm_conc_connection_list[1].mode == PM_STA_MODE) && pm_conc_connection_list[1].in_use) {
				  link1_work_freq = pm_conc_connection_list[1].freq;
				  sap_debug("allow sap to use link1_work_freq %u", link1_work_freq);
			 }
		}
	...
	for (loop_count = band_start_ch; loop_count <= band_end_ch;
		 ...
		 */
		if (mac_ctx->pdev && link0_work_freq != chan_freq && link1_work_freq != chan_freq &&
				is_sap_only_allow_sta_dfs_indoor_chan &&
