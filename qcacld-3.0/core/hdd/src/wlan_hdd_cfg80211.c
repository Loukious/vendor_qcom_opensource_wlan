...
static bool is_sap_fast_acs = false;
#endif
...
// MIUI ADD: WIFI_BugFixUpstream
#ifdef WLAN_FEATURE_FAST_SAP_ACS
static int separate_24g_5g_freq(uint32_t *raw_data, uint32_t raw_data_len, uint32_t *freq_list_24g,
				uint32_t *freq_list_24g_len, uint32_t *freq_list_5g,
...
// Scenario 2: Select a channel based on the bandwidth
static int hdd_get_usable_channel(struct hdd_context *hdd_ctx, const void *data, uint32_t dataLen,
				int ch_width, uint32_t *list_usable_2g, uint32_t *list_usable_2g_count,
				uint32_t *list_usable_5g, uint32_t *list_usable_5g_count) {
	...
	if (!tb[QCA_WLAN_VENDOR_ATTR_USABLE_CHANNELS_BAND_MASK]) {
		...
		req_msg.band_mask = (1 << REG_BAND_2G) | (1 << REG_BAND_5G) | (1 << REG_BAND_6G);
	} else {
		req_msg.band_mask =
		...
		if (!req_msg.band_mask)
			...
			if (req_msg.band_mask & 1 << NL80211_BAND_2GHZ)
				req_msg.band_mask |= 1 << REG_BAND_2G;
			if (req_msg.band_mask & 1 << NL80211_BAND_5GHZ)
				req_msg.band_mask |= 1 << REG_BAND_5G;
			if (req_msg.band_mask & 1 << NL80211_BAND_6GHZ)
				req_msg.band_mask |= 1 << REG_BAND_6G;
			if (req_msg.band_mask & 1 << NL80211_BAND_60GHZ)
	...
	if (!tb[QCA_WLAN_VENDOR_ATTR_USABLE_CHANNELS_IFACE_MODE_MASK]) {
		...
		req_msg.iface_mode_mask = (1 << NL80211_IFTYPE_STATION) |
				(1 << NL80211_IFTYPE_AP) |
	...
	status = wlan_reg_get_usable_channel(hdd_ctx->pdev, req_msg,
                                             res_msg, &count, REG_CURRENT_PWR_MODE);
	...
	separate_24g_5g_freq(sap_cfg_tmp, sap_cfg_tmp_count,
			list_usable_2g, list_usable_2g_count, list_usable_5g, list_usable_5g_count);

	qdf_mem_free(res_msg);
	return sap_cfg_tmp_count;
}

// Scenario 3: According to the wifi connection channel, select SAP channel
static int hdd_set_acs_freq_same_as_wifi(struct hdd_context *hdd_ctx) {
	...
	// prefer scc when wifi is connected
	if ((policy_mgr_is_hw_dbs_capable(hdd_ctx->psoc) && WLAN_REG_IS_5GHZ_CH_FREQ(freq_sta_connected)) ||
			(!policy_mgr_is_hw_dbs_capable(hdd_ctx->psoc) &&
...
// update SAP ACS channel info by the above method
static int hdd_update_acs_info(struct hdd_context *hdd_ctx,
				struct sap_config *sap_cfg, const void *data, uint32_t dataLen) {
	...
	// check SAP ch wether can work on wifi ch
	...
	hdd_get_usable_channel(hdd_ctx, data, dataLen,
			sap_cfg->acs_cfg.ch_width, freq_list_usable_2g, &freq_list_usable_2g_count,
			freq_list_usable_5g, &freq_list_usable_5g_count);

	separate_24g_5g_freq(sap_cfg->acs_cfg.freq_list, sap_cfg->acs_cfg.ch_list_count,
			list_sap_cfg_2g, &list_sap_cfg_2g_count, list_sap_cfg_5g, &list_sap_cfg_5g_count);

	for (i = 0; i < freq_list_usable_2g_count; i++) {
		if (list_sap_cfg_2g_count && freq_list_usable_2g[i] > list_sap_cfg_2g[0] &&
	...
	for (j = 0; j < freq_list_usable_5g_count; j++) {
		if (list_sap_cfg_5g_count && freq_list_usable_5g[j] >= list_sap_cfg_5g[0] &&
...
out:
	// check acs channel is safe, not conflict with LTE
	if (!hdd_is_acs_channel_safe_channel(hdd_ctx, freq_list_tmp_count, freq_list_tmp)) {
		hdd_debug("No safe channel, do nothing");
		return -1;
	}

	for (ori_freq_index = 0; ori_freq_index < freq_list_tmp_count; ori_freq_index++) {
		for (pcl_index = 0; pcl_index < sap_cfg->acs_cfg.pcl_ch_count; pcl_index++) {
			if (sap_cfg->acs_cfg.pcl_chan_freq[pcl_index] == freq_list_tmp[ori_freq_index]) {
				freq_in_pcl = true;
				break;
			}
		}
	...
	for (freq_tmp_index = 0; freq_tmp_index < sap_cfg->acs_cfg.ch_list_count; freq_tmp_index++) {
		if (freq_tmp_index < freq_list_tmp_count)
			sap_cfg->acs_cfg.freq_list[freq_tmp_index] = freq_list_tmp[freq_tmp_index];
		else
...
// MIUI ADD: WIFI_BugFixUpstream
#ifdef WLAN_FEATURE_FAST_SAP_ACS
static int wlan_hdd_cfg80211_set_fast_acs(struct wlan_hdd_link_info *link_info,
				const struct nlattr *attr) {
