...
const char *wlan_get_cmdline(void)
{
	...
	if (of_chosen) {
		...
		if (!bootargs) {
			hdd_info("%s: failed to get bootargs\n", __func__);
                }
		else {
			strcpy(__platform_cmdline, bootargs);
		}
	} else {
		hdd_info("%s: failed to get /chosen\n", __func__);
	}
	return command_line;
}
...
QDF_STATUS hdd_update_mac_config(struct hdd_context *hdd_ctx)
{
	...
	if (!hdd_ctx->config->read_mac_addr_from_mac_file ||
	  strstr(command_line, "icnss2.hwname=ruan")) {
		hdd_info("Reading MAC address from MAC file is not enabled.");
		return QDF_STATUS_E_FAILURE;
	} else {
		hdd_info("Reading MAC address from MAC file is enabled.");
	}
	...
	if (hdd_ctx->num_provisioned_addr)
		...
		qdf_mem_copy(&custom_mac_addr,
			     ...
			     sizeof(tSirMacAddr));
	if (custom_mac_addr[0] & 0x03) {
		custom_mac_addr[0] &= 0xfc;  // clear last 2 bits
		hdd_info("Invalid mac addr detected, force the first byte valid!");
	}
	sprintf(wifi_mac_addr, "%02x:%02x:%02x:%02x:%02x:%02x", custom_mac_addr[0], custom_mac_addr[1],
		custom_mac_addr[2], custom_mac_addr[3], custom_mac_addr[4], custom_mac_addr[5]);
	hdd_info("Read wlan mac: %s", wifi_mac_addr);
