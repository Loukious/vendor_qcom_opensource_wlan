				      ...
				      int nss, int mode)
{
	...
	if ((nss == NSS_1x1_MODE && mode == HDD_ANTENNA_MODE_2X2) ||
		...
		hdd_err("nss and antenna mode mismatch: nss=%d mode=%d",
			nss, mode);
		return -EINVAL;
	}
	...
	if (QDF_IS_STATUS_ERROR(status)) {
		hdd_err("cfg set antenna mode failed, value %d status %d",
			mode, status);
		return qdf_status_to_os_return(status);
	}
...
void wlan_hdd_stop_nss_mode_switch(struct hdd_adapter *adapter)
{
	...
	if (adapter->device_mode == QDF_STA_MODE) {
		if (g_nss_mode_type == NSS_MODE_2x2 ||
		    g_nss_mode_type == NSS_MODE_DEFAULT) {
			g_nss_mode_type = NSS_MODE_DEFAULT;
			goto exit;
		}

		wlan_hdd_set_nss_and_antenna_mode(adapter->deflink,
						  NSS_2x2_MODE,
