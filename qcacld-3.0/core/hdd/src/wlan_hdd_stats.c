...
#define HDD_INACTIVE_LINK_RSSI -96
#define HDD_INACTIVE_LINK_SNR 0
				 ...
				 struct station_info *sinfo)
{
	...
	if (!link_info->rssi) {
	    ...
	    if (policy_mgr_vdev_is_force_inactive(psoc,
				  link_info->vdev_id)) {
		link_info->rssi = HDD_INACTIVE_LINK_RSSI;
		snr = HDD_INACTIVE_LINK_SNR;
	} else {
