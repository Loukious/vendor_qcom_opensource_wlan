...
#define ELF_BDF_FILE_NAME_O2_V1		"bd_o2_v1.elf"
#define ELF_BDF_FILE_NAME_O2_GL_V1	"bd_o2gl_v1.elf"
#define ELF_BDF_FILE_NAME_O3_V1		"bd_o3_v1.elf"
#define ELF_BDF_FILE_NAME_O3_GL_V1	"bd_o3gl_v1.elf"
#define ELF_BDF_FILE_NAME_O11U_V1	"bd_o11u_v1.elf"
#define ELF_BDF_FILE_NAME_O11U_GL_V1	"bd_o11ugl_v1.elf"
#define ELF_BDF_FILE_NAME_O2		"bd_o2.elf"
#define ELF_BDF_FILE_NAME_O2_GLOBAL	"bd_o2gl.elf"
#define ELF_BDF_FILE_NAME_O2_INDIA	"bd_o2in.elf"
#define ELF_BDF_FILE_NAME_O3		"bd_o3.elf"
#define ELF_BDF_FILE_NAME_O3_GLOBAL	"bd_o3gl.elf"
#define ELF_BDF_FILE_NAME_O3_INDIA	"bd_o3in.elf"
#define ELF_BDF_FILE_NAME_O11U		"bd_o11u.elf"
#define ELF_BDF_FILE_NAME_O11U_GLOBAL	"bd_o11ugl.elf"
		...
		if (plat_priv->board_info.board_id == 0xFF) {
			if (plat_priv->chip_info.chip_id & CHIP_ID_GF_MASK)
				...
				if (plat_priv->device_version.major_version == FW_V1_NUMBER) {
					if (hw_platform_ver == HARDWARE_PROJECT_O2) {
						if ((uint32_t)CountryGlobal == hw_country_ver)
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O2_GL_V1);
						else
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O2_V1);
					} else if (hw_platform_ver == HARDWARE_PROJECT_O3) {
						if ((uint32_t)CountryGlobal == hw_country_ver)
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O3_GL_V1);
						else
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O3_V1);
					} else if (hw_platform_ver == HARDWARE_PROJECT_O11U) {
						if ((uint32_t)CountryGlobal == hw_country_ver)
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O11U_GL_V1);
						else
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O11U_V1);
					}
					else
					snprintf(filename_tmp, filename_len,
						 ELF_BDF_FILE_NAME);
					...
					if (hw_platform_ver == HARDWARE_PROJECT_O2) {
						if ((uint32_t)CountryGlobal == hw_country_ver)
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O2_GLOBAL);
						else if ((uint32_t)CountryIndia == hw_country_ver)
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O2_INDIA);
						else
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O2);
					} else if (hw_platform_ver == HARDWARE_PROJECT_O3) {
						if ((uint32_t)CountryGlobal == hw_country_ver)
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O3_GLOBAL);
						else if ((uint32_t)CountryIndia == hw_country_ver)
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O3_INDIA);
						else
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O3);
					} else if (hw_platform_ver == HARDWARE_PROJECT_O11U) {
						if ((uint32_t)CountryGlobal == hw_country_ver)
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O11U_GLOBAL);
						else
						snprintf(filename_tmp, filename_len,
							ELF_BDF_FILE_NAME_O11U);
					}
					else
					snprintf(filename_tmp, filename_len,
						 ELF_BDF_FILE_NAME);
