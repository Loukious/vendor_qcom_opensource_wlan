...
#define ELF_BDF_FILE_NAME_O81		"bd_o81.elf"
#define ELF_BDF_FILE_NAME_O81_GLOBAL	"bd_o81gl.elf"
#define ELF_BDF_FILE_NAME_O11		"bd_o11.elf"
#define ELF_BDF_FILE_NAME_O11_GLOBAL	"bd_o11gl.elf"
	...
	case CNSS_BDF_ELF:
		...
		if (plat_priv->board_info.board_id == 0xFF) {
			if (plat_priv->chip_info.chip_id & CHIP_ID_GF_MASK)
				...
				else if (hw_platform_ver == HARDWARE_PROJECT_O81) {
					if((uint32_t)CountryGlobal == hw_country_ver)
						snprintf(filename_tmp, filename_len, ELF_BDF_FILE_NAME_O81_GLOBAL);
					else
						snprintf(filename_tmp, filename_len, ELF_BDF_FILE_NAME_O81);
				} else if (hw_platform_ver == HARDWARE_PROJECT_O11) {
					if((uint32_t)CountryCN == hw_country_ver)
						snprintf(filename_tmp, filename_len, ELF_BDF_FILE_NAME_O11);
					else
						snprintf(filename_tmp, filename_len, ELF_BDF_FILE_NAME_O11_GLOBAL);
				}
				else
				snprintf(filename_tmp, filename_len,
					 ELF_BDF_FILE_NAME);
