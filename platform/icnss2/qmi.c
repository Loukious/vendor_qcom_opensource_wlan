...
#define ELF_BDF_FILE_NAME_O16U          "bd_o16u.elf"
#define ELF_BDF_FILE_NAME_O16U_INDIA    "bd_o16uin.elf"
#define ELF_BDF_FILE_NAME_O16U_GLOBAL	"bd_o16ugl.elf"
#define ELF_BDF_FILE_NAME_O82		"bd_o82.elf"
#define ELF_BDF_FILE_NAME_O82_INDIA	"bd_o82in.elf"
#define ELF_BDF_FILE_NAME_O82_GLOBAL	"bd_o82gl.elf"
#define ELF_BDF_FILE_NAME_P16U          "bd_p16u.elf"
#define ELF_BDF_FILE_NAME_P16U_INDIA    "bd_p16uin.elf"
#define ELF_BDF_FILE_NAME_P16U_GLOBAL	"bd_p16ugl.elf"
				   ...
				   u32 filename_len)
{
	...
	case ICNSS_BDF_ELF:
		if (priv->board_id == 0xFF)
			...
			} else if (hw_platform_ver == HARDWARE_PROJECT_O16U) {
                                if ((uint32_t)CountryIndia == hw_country_ver)
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_O16U_INDIA);
				else if ((uint32_t)CountryGlobal == hw_country_ver)
				snprintf(filename_tmp, filename_len,
					ELF_BDF_FILE_NAME_O16U_GLOBAL);
				else
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_O16U);
			} else if (hw_platform_ver == HARDWARE_PROJECT_O82) {
                                if ((uint32_t)CountryIndia == hw_country_ver)
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_O82_INDIA);
				else if ((uint32_t)CountryGlobal == hw_country_ver)
				snprintf(filename_tmp, filename_len,
					ELF_BDF_FILE_NAME_O82_GLOBAL);
				else
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_O82);

			} else if (hw_platform_ver == HARDWARE_PROJECT_P16U) {
                                if ((uint32_t)CountryIndia == hw_country_ver)
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_P16U_INDIA);
				else if ((uint32_t)CountryGlobal == hw_country_ver)
				snprintf(filename_tmp, filename_len,
					ELF_BDF_FILE_NAME_P16U_GLOBAL);
				else
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_P16U);
			} else {
