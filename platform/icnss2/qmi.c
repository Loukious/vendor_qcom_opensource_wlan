...
#define BDF_FILE_P17_CN 	"bd17cn.bin"
#define BDF_FILE_P17_IN 	"bd17in.bin"
#define BDF_FILE_P17_IP 	"bd17ip.bin"
#define BDF_FILE_P17_GS 	"bd17gs.bin"
#define BDF_FILE_P17_GP 	"bd17gp.bin"
#define BDF_FILE_P17_GL 	"bd17gl.bin"
...
/* lct.caoxiangrui modify for BUGP17-1070 begin */
#define MAX_RETRIES 10
#define RETRY_DELAY_MS 100
/* lct.caoxiangrui modify for BUGP17-1070 end   */
...
int wlfw_cap_send_sync_msg(struct icnss_priv *priv)
{
	...
	icnss_pr_info("RD card chain cap: %d, PHY HE channel width cap: %d, PHY QAM cap: %d",
	...
	case ICNSS_BDF_ELF:
		if (priv->board_id == 0xFF)
			...
			snprintf(filename_tmp, filename_len,
				 ...
				 priv->board_id & 0xFF);
		break;
        //lct.dongxiaolong modify for get sku begin
	case ICNSS_BDF_BIN:
		if(priv->chip_info.chip_family == WCN3988_Family_ID){
			icnss_wlan_get_cmdline();
				if (strstr(command_line, "wlancmdline.sku=rin")){
					memset(fname, 0, sizeof(filename_tmp));
					strcpy(fname,BDF_FILE_P17_IN);
					scnprintf(filename_tmp, ICNSS_MAX_FILE_NAME, "%s", fname);
					icnss_pr_info("Use India bd17inx.bin bdf\n");
				}
				else if (strstr(command_line, "wlancmdline.sku=rgl")){
					memset(fname, 0, sizeof(filename_tmp));
					strcpy(fname,BDF_FILE_P17_GL);
					scnprintf(filename_tmp, ICNSS_MAX_FILE_NAME, "%s", fname);
					icnss_pr_info("Use Global bd17glx.bin bdf\n");
				}
				else if (strstr(command_line, "wlancmdline.sku=rcn")){
					memset(fname, 0, sizeof(filename_tmp));
					strcpy(fname,BDF_FILE_P17_CN);
					scnprintf(filename_tmp, ICNSS_MAX_FILE_NAME, "%s", fname);
					icnss_pr_info("Use China bd17cnx.bin bdf\n");
				}
				else if (strstr(command_line, "wlancmdline.sku=pgl")){
					memset(fname, 0, sizeof(filename_tmp));
					strcpy(fname,BDF_FILE_P17_GP);
					scnprintf(filename_tmp, ICNSS_MAX_FILE_NAME, "%s", fname);
					icnss_pr_info("Use Global POCO bd17gpx.bin bdf\n");
				}
				else if (strstr(command_line, "wlancmdline.sku=rsa")){
					memset(fname, 0, sizeof(filename_tmp));
					strcpy(fname,BDF_FILE_P17_GS);
					scnprintf(filename_tmp, ICNSS_MAX_FILE_NAME, "%s", fname);
					icnss_pr_info("Use Global Saudi bd17gsx.bin bdf\n");
				}
				else if (strstr(command_line, "wlancmdline.sku=pin")){
					memset(fname, 0, sizeof(filename_tmp));
					strcpy(fname,BDF_FILE_P17_IP);
					scnprintf(filename_tmp, ICNSS_MAX_FILE_NAME, "%s", fname);
					icnss_pr_info("Use India POCO bd17ipx.bin bdf\n");
				}
				else {
					memset(fname, 0, sizeof(filename_tmp));
					...
					icnss_pr_info("Hwname=kunzite,unknow hwc,use default bdf\n");
				}
		}
                //lct.dongxiaolong modify for get sku end
